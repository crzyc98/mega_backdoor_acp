...
def determine_acp_inclusion(
    *,
    ...
    A participant may be excluded from ACP only if they were never eligible
    to make after-tax contributions or receive match during the plan year.
    Zero contributions alone are NOT grounds for exclusion.

    Logic:
    1. Compute milestone dates:
       - age21_date = DOB + 21 years
       - yos1_date = hire_date + 1 year (elapsed time approximation)
       - eligibility_date = max(age21_date, yos1_date)

    2. Compute entry date (semi-annual):
       - Entry dates are Jan 1 and Jul 1
       - entry_date = first Jan 1 or Jul 1 on or after eligibility_date

    3. Determine participation window:
       - participation_start = entry_date
       - participation_end = termination_date if present, else plan_year_end

    4. ACP includable test:
       - participation_start <= plan_year_end AND
       - (termination_date is null OR termination_date >= participation_start)

    Args:
        dob: Date of birth (required)
        hire_date: Hire date (required)
        termination_date: Termination date (optional)
        plan_year_start: Start of plan year
        plan_year_end: End of plan year

    Returns:
        ACPInclusionResult with eligibility details

    Raises:
        ACPInclusionError: If required date fields are missing or invalid
    """
