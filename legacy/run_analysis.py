...
def display_detailed_calculations(results_df, adoption_rates, contribution_rates):
    ...
    print("=" * 100)
    print("Formula: ACP = Average of individual employee rates")
    print("Individual Rate = (Matching + After-Tax Contributions ÷ Compensation) × 100")
    print("IRS Test: HCE ACP ≤ (NHCE ACP × 1.25) OR (NHCE ACP + 2.0)")
    ...
    for adopt_rate in adoption_rates:
        for contrib_rate in contribution_rates:
            ...
            print(f"   │  └─ ACP: Average of individual rates = {scenario['nhce_acp']:.3f}%")
            print(f"   │")
            ...
            print(f"   │  └─ ACP: Average of individual rates = {scenario['hce_acp']:.3f}%")
            print(f"   │")
            print(f"   └─ IRS Test Results:")
            print(f"      ├─ Limit A: {scenario['nhce_acp']:.3f}% × 1.25 = {scenario['limit_a']:.3f}% {'✓' if scenario['passed_limit_a'] else '✗'}")
            print(f"      ├─ Limit B: {scenario['nhce_acp']:.3f}% + 2.0 = {scenario['limit_b']:.3f}% {'✓' if scenario['passed_limit_b'] else '✗'}")
            print(f"      ├─ Test: Pass if HCE ACP ≤ either limit")
            print(f"      ├─ HCE ACP: {scenario['hce_acp']:.3f}%")
